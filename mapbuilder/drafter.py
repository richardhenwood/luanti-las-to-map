...
class Writer():
    ...
    def bedrock(self, depth, ztop): #ztop is the max z
        bedrockblock = []
        blockdim = 16
        ...
        with sqlite3.connect(self.filename) as conn:
            ...
            cursor.executemany(
                'INSERT INTO blocks (x, y, z, data) VALUES (?, ?, ?, ?)',
                        self._bedrock_block(blob_bytes))
            conn.commit()

    def _bedrock_block(self, blob_bytes):
        dim = 4096
        for x in range(dim):
            for y in range(dim):
                for z in range(ztop - depth, ztop):
                    yield (x, 
                        z, 
                        y, 
                        blob_bytes))
            logger.info(f"writing bedrock {x} of {dim}")
