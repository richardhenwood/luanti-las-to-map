...
class LasBlueprint(Blueprint):
    ...
    def _preprocess(self):
        with laspy.open(self.datafile) as fh:
            ...
            xhist = numpy.histogram(xdiff) # use 10 bins
            ...
            self.xres = xhist[1][xresindex] * 4096 * 16
            ...
            self.yres = yhist[1][yresindex] * 4096 * 16
    ...
    def _get_las_point(self, las, pidx):
        # return a normalized point x,y: 0->1
        # leave z absolute
        x = int(((las.x[pidx] - self.abs_x_min) / (self.abs_x_max - self.abs_x_min)) * self.xres)
        y = int(((las.y[pidx] - self.abs_y_min) / (self.abs_y_max - self.abs_y_min)) * self.yres)
        #z = int(((las.z[pidx] - self.abs_z_min) / (self.abs_z_max - self.abs_z_min)) / self.z_import_scale)
        z = int(las.z[pidx] * self.z_import_scale)# - self.abs_z_min) / (self.abs_z_max - self.abs_z_min)) / self.z_import_scale)
        c = las.classification[pidx]
        return x, y, z, Reference.classification[c][1] #"default:dirt" #las.classification[pidx]
